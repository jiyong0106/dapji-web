...
import {
  useCreateSyncPost,
  useFetchInstaUsers,
  useFetchUserShortcodes,
  useSendInstaSyncNotification,
} from '../api';
...
const InstaSyncPage = () => {
  ...
  // 동기화 성공 시 호출될 콜백 함수
  const handleSyncSuccess = (shortcode: string) => {
    setRecentlySynced((prev) => new Set(prev).add(shortcode));
  };
  ...
  const { mutate: createSyncPost, isPending } =
    useCreateSyncPost(handleSyncSuccess);
  const { mutate: sendNotification, isPending: isNotifying } =
    useSendInstaSyncNotification();

  // 사용자 선택 시, 최근 동기화 목록 초기화
  const handleUserSelect = (instaName: string) => {
    setSelectedInstaName(instaName);
    setSelectedShortcode('');
    setRecentlySynced(new Set()); // 다른 사용자를 선택하면 초기화
  };

  // 새로고침 시, 최근 동기화 목록 초기화
  const handleFetchShortcodes = () => {
    if (selectedInstaName) {
      refetchShortcodes();
      setRecentlySynced(new Set()); // 새로고침 시 초기화
    }
  };
  ...
  const handleSync = (shortcode: string) => {
    ...
    if (shortcodesData && shortcodesData.user_idx) {
      console.log(
        `숏코드 ${shortcode} 동기화 시작 (사용자 ID: ${shortcodesData.user_idx})`,
      );
  ...
  return (
    // isPending일 때 'loading' 클래스를 추가하여 자식 요소들의 이벤트를 막을 수 있습니다.
    <div className={cn('container', { loading: isPending })}>
      {/* 로딩 오버레이: isPending이 true일 때만 표시됩니다. */}
      {isPending && (
      ...
      <div className={cn('content-container')}>
        <div className={cn('users-list')}>
          ...
          <div className={cn('user-cards')}>
            {usersData?.data.map((user: any) => (
              <div
                key={user.user_idx}
                className={cn('user-card', {
                  active: selectedInstaName === user.insta_name,
                })}
        ...
        {selectedInstaName ? (
              ...
              <div className={cn('action-buttons')}>
                <button
                  ...
                  disabled={
                    ...
                    isPending ||
                    !shortcodesData ||
                    Object.keys(shortcodesData.shortcodes_with_dates || {})
                      .length === 0
                  }
                >
            ...
            {isShortcodesFetching ? (
              ...
              shortcodesData.is_valid ? (
                <div className={cn('shortcodes-container')}>
                  {Object.keys(shortcodesData.shortcodes_with_dates).length >
                  0 ? (
                    <>
                      ...
                      <div className={cn('shortcodes-grid')}>
                        {Object.entries(
                          shortcodesData.shortcodes_with_dates,
                        ).map(([shortcode, date]: [string, any]) => {
                          // 방금 동기화된 항목인지 확인
                          const isJustSynced = recentlySynced.has(shortcode);

                          return (
                            // 방금 동기화된 항목에 'just-synced' 클래스 추가
                            <div
                              key={shortcode}
                              className={cn('shortcode-item', {
                                'just-synced': isJustSynced,
                              })}
                            >
                              ...
                              <button
                                ...
                                // isPending 상태이거나, 방금 동기화된 항목이면 비활성화
                                disabled={
                                  (isPending &&
                              ...
                              >
                                {isPending && selectedShortcode === shortcode
                                  ? '처리중'
                                  : isJustSynced
                                    ? '완료됨 ✓' // 방금 동기화된 항목의 텍스트 변경
                                    : '동기화'}
                              </button>
                            </div>
                          );
                  ...
                  {Object.keys(shortcodesData.synced_shortcodes).length > 0 && (
                    <>
                      <h3 className={cn('section-title', 'synced-title')}>
                        동기화 완료 (
                        {Object.keys(shortcodesData.synced_shortcodes).length}
                        개)
                      </h3>
                      <div className={cn('shortcodes-grid', 'synced')}>
                        {Object.entries(shortcodesData.synced_shortcodes).map(
                          ([shortcode, data]: [string, any]) => {
                            ...
                            const syncedPostId =
                              typeof data === 'object'
                                ? data.insta_sync_post_idx
                                : parseInt(shortcode.replace(/\D/g, '')) || 1;
                            // 알림 발송 여부 및 게시글 연동 여부
                            const notificationSent =
                              typeof data === 'object'
                                ? data.notification_sent
                                : false;
                            const convertedToPost =
                              typeof data === 'object'
                                ? data.converted_to_post
                                : false;
                            const postExists =
                              typeof data === 'object'
                                ? data.post_exists
                            ...
                            return (
                              <div
                                key={shortcode}
                                className={cn('shortcode-item', 'synced')}
                              >
                                <div className={cn('shortcode-info')}>
                                  <p className={cn('shortcode')}>{shortcode}</p>
                                  <span className={cn('date-small')}>
                                    {date}
                                  </span>
                                  <div className={cn('status-labels')}>
                                    <span
                                      className={cn('status-label', {
                                        'status-yes': notificationSent,
                                        'status-no': !notificationSent,
                                      })}
                                    >
                                      알림 발송 여부:{' '}
                                      {notificationSent ? 'O' : 'X'}
                                    </span>
                                    <span
                                      className={cn('status-label', {
                                        'status-yes':
                                          convertedToPost && postExists,
                                        'status-no': !(
                                          convertedToPost && postExists
                                        ),
                                      })}
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    handleSendNotification(syncedPostId)
                                  }
                                  disabled={
                                    isNotifying &&
                                    notifyingPostId === syncedPostId
                                  }
                                >
                                  {isNotifying &&
                                  notifyingPostId === syncedPostId
              ...
              <div className={cn('empty-state')}>
                <p>
                  계정을 선택하고 새로고침 버튼을 클릭하면 동기화 목록이
                  표시됩니다.
                </p>
              </div>
            )}
          ...
          <div className={cn('user-details', 'empty')}>
            <p className={cn('empty-message')}>
              좌측에서 인스타그램 계정을 선택해주세요.
            </p>
          </div>
        )}
      </div>
